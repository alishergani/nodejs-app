...
function autocomplete(input, lng, lat) {
  ...
  input.addEventListener('input', function () {
    ...
    if (this.value !== "") {
      _axios2.default.get('https://api.mapbox.com/geocoding/v5/mapbox.places/' + this.value + '.json?access_token=pk.eyJ1Ijoia2E0YW5iZWsiLCJhIjoiY2pvcHhpcW5uMDZoZTNya3dpdXkzaDQ3eCJ9.urptUPgdj8mxbDckQVPitA&autocomplete=true').then(function (res) {
        var places = [];
        res.data.features.map(function (place) {
          return places.push(place);
        });
        return places;
      }).then(function (res) {
        document.querySelector('.address_dropdown').innerHTML = searchResultsHTML(res);
        var items = document.querySelectorAll('.address_dropdown p.item');
...
function hero(elem) {
	var letterArr = 'Meldebek Alisher'.split('');
	var textArr = [];

	letterArr.map(function (letter, i) {
	...
	var letterList = document.querySelectorAll('.hero span');
	letterList.map(function (item, i) {
		setTimeout(function () {
...
function typeAhead(search) {
	...
	searchInput.on('input', function (e) {
		...
		_axios2.default.get('/api/search?q=' + this.value).then(function (res) {
			...
			searchResults.innerHTML = '<div class="search__result">No \n\t\t\t\tresults for ' + _this.value + ' found!</div>';
		}).catch(function (err) {
			console.error(err);
		});
	});
...
if ((0, _bling.$)('.flash')) {
	setTimeout(function () {
		(0, _bling.$)('.flash').classList.add('fade-out');
	}, 1000);
}

(0, _hero2.default)((0, _bling.$)('.hero'));
...
function loadPlaces(map) {
	var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 43.2363513784371;
	var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 76.9457906484604;
}
